...
/* JESS: 
 * Read the comments above the functions to understand what they do.
 * Don't try to understand the code in this file.  It will make you sad. :( 
 */
...
bool runProcess( char *pName, char **ppSystemCall, int *pNumSteps ){
    ...
    *ppSystemCall = NULL;
    ...
    if( *pNumSteps==INT_MAX )
        printf( "Attempting to run process: %s-%d until it completes\n", RAM->TLN, RAM->heap[map[ZERO]] );
    else
...
void promoteProcess( char *pName, processData *pData ){
    if( authentication( pName, pData ) ){
        if( pData->heap[map[SEVEN]] ) {
            ...
            if( (step-pData->heap[map[ELEVEN]]-pData->heap[map[TEN]])>=0 )
